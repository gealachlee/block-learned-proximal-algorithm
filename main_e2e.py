...
# from prox.prox_cl import ProxL2_1, ProxL2_1over2,ProxL2_2over3,ProxMCP,ProxCappedl1
...
def make_train_step(model, optimizer):
    def train_step(d, network_layer, x_gt=None):
        ...
        loss = objective_val(x_pred, d, x_gt, A=ATEN)#,WTA = model.W[network_layer-1].T@model.A)
        loss.backward()
        # print(model.theta[0])
        optimizer.step()
        optimizer.zero_grad()
...
model = ModelFactory().create(
    ...
    device=opts.device,
    dtype=opts.dtype,
    prox_cl=ProxL2_1over2(n=opts.n, gLen=opts.gLen)
)
