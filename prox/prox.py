...
warnings.warn('Prox is abandand,please use prox_cl_acc instead')

def prox21(x: torch.Tensor, v: torch.Tensor):
...
def operator_approximate(x: torch.Tensor, v: torch.Tensor):
    num_subvectors = 512 // 16
    x_subvectors = torch.chunk(x, num_subvectors, dim=1)
...
def l2_2over3(x: torch.Tensor, v: torch.Tensor):
    ...
    norms = torch.where(
        ...
        torch.sqrt(torch.sqrt(8 * 256 * (v ** 3) / 729)) + 1e-10,
        norms)
